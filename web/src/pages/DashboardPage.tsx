...
import { EventsPagination } from '../components/table/EventsPagination'
...
export function DashboardPage() {
  ...
  const eventsQuery = useEvents(range, page, EVENTS_PAGE_SIZE)

  function handleRangeChange(nextRange: DateRangePreset) {
    setRange(nextRange)
    setPage(0)
  }
  const isEmpty =
    ...
    <main className="min-h-screen bg-slate-50 px-6 py-12 text-slate-900">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-8 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {eventsQuery.isError && (
            <div className="mt-4">
              <p className="font-medium text-red-700">Could not load usage events</p>
              <p className="mt-1 text-sm text-slate-600">
          ...
          {!eventsQuery.isError && eventsQuery.isSuccess && eventsQuery.data.total === 0 && (
            <p className="mt-4 text-sm text-slate-600">
              No usage events in this period.
            </p>
          )}
